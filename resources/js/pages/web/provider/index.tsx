...
function SeeAllAreasModal({ open, onClose, selected, onApply }) {
    ...
    const AreaBtn = ({ area }) => {
        const active = local.includes(area);
        return (
            <button type="button" onClick={() => toggle(area)} aria-pressed={active}
            ...
            <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-xl sm:rounded-2xl">
                ...
                <div className="border-b border-[#EFEAE0] p-4 sm:p-5">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg text-[#16302F]" style={SERIF}>Areas of support</h3>
                    ...
                    <div className="relative mt-3">
                        ...
                        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search all areas of support…"
                    ...
                    {searching ? (
                        <div className="p-4 sm:p-5">
                            {searchResults.length === 0
                                ...
                                : <div className="grid gap-2 sm:grid-cols-2">{searchResults.map((a) => <AreaBtn key={a} area={a} />)}</div>}
                        </div>
                    ) : (
                        <>
                            {/* desktop: two columns */}
                            <div className="hidden sm:flex">
                                <ul className="w-56 flex-shrink-0 border-r border-[#EFEAE0] p-2">
                                ...
                                <div className="grid flex-1 content-start gap-2 p-4 sm:grid-cols-2">
                                    {activeItems.map((a) => <AreaBtn key={a} area={a} />)}
                                </div>
                            </div>
                            {/* mobile: accordion */}
                                ...
                                {AREAS_OF_SUPPORT_GROUPS.map((g) => {
                                    ...
                                    return (
                                        <div key={g.category} className="border-b border-[#EFEAE0]">
                                            <button type="button" onClick={() => setOpenMobileCat(isOpen ? '' : g.category)}
                                                className="flex w-full items-center justify-between gap-2 px-4 py-3.5 text-left">
                                                <span className="flex items-center gap-2 text-sm font-medium text-[#16302F]">
                                                    {g.category}
                                                    {c > 0 && <span className="rounded-full bg-[#0E7C7B] px-1.5 py-0.5 text-[10px] font-semibold text-white">{c}</span>}
                                                </span>
                                            ...
                                            {isOpen && <div className="grid gap-2 px-4 pb-4">{g.items.map((a) => <AreaBtn key={a} area={a} />)}</div>}
                                        </div>
                                    );
                                })}
                            </div>
                ...
                <div className="flex items-center justify-between gap-3 border-t border-[#EFEAE0] p-4 sm:p-5">
                    <div className="flex items-center gap-3 text-sm text-[#5B6B6E]">
                        ...
                        {local.length > 0 && <button onClick={() => setLocal([])} className="underline underline-offset-2 hover:text-[#0E7C7B]">Clear all</button>}
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => onApply(local)} className="rounded-xl bg-[#0E7C7B] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#0B6463]">Apply selections</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    if (availability === 'accepting') {
        return (
            <p className="flex flex-wrap items-center gap-x-2 font-medium text-[#0E6B6A]">
                <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#0E7C7B]" /> Accepting new clients</span>
            </p>
        );
    }
    if (availability === 'not_accepting') {
        return <p className="text-[#8A9795]">Not currently accepting new clients</p>;
    }
    return <p className="text-[#8A9795]">Contact the provider directly to confirm current availability.</p>;
}
    ...
    return (
        <article className="group flex flex-col rounded-2xl border border-[#E7E0D2] bg-white p-5 shadow-[0_1px_2px_rgba(20,20,20,0.04)] transition hover:-translate-y-0.5 hover:border-[#DCD0BA] hover:shadow-[0_10px_30px_-12px_rgba(14,124,123,0.25)]">
            <div className="flex items-start gap-4">
                {showPhoto ? (
                    <img src={p.photo} alt={p.name} onError={() => setImgError(true)} className="h-16 w-16 flex-shrink-0 rounded-xl object-cover ring-1 ring-black/5" />
                ) : (
                    <div aria-hidden className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#0E4C4B] to-[#0E7C7B] font-serif text-lg text-white/90 ring-1 ring-black/5" style={SERIF}>{initials(p.name)}</div>
                )}
                    ...
                    <p className="mt-0.5 flex flex-wrap items-center gap-x-2 text-sm text-[#5B6B6E]">
                        ...
                        {p.verified && (
                            <span className="inline-flex items-center gap-1 text-[13px] font-medium text-[#0E6B6A]">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden><path d="M9 12.75 11.25 15 15 9.75m6 2.25a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        ...
                        <p className="mt-1 flex flex-wrap items-center gap-x-2 text-xs text-[#6B7A78]">
                            {p.location && (
                                <span className="inline-flex items-center gap-1">
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path d="M12 21s-6-5.686-6-10a6 6 0 1 1 12 0c0 4.314-6 10-6 10Z" /><circle cx="12" cy="11" r="2" /></svg>
                                    {p.location}
                                </span>
                            )}
            ...
            {(specialties.length > 0 || populations.length > 0 || p.languages?.length > 0) && (
                <div className="mt-4 space-y-1.5 text-sm text-[#33302a]">
                    {specialties.length > 0 && <p><span className="font-semibold text-[#3A4B49]">Can help with:</span> {specialties.join('  •  ')}</p>}
                    {populations.length > 0 && <p className="text-[#5B6B6E]"><span className="font-semibold text-[#3A4B49]">Works with:</span> {populations.join('  •  ')}</p>}
                    {p.languages?.length > 0 && <p className="text-[#5B6B6E]"><span className="font-semibold text-[#3A4B49]">Languages:</span> {p.languages.join(', ')}</p>}
                </div>
            )}
            ...
            <div className="mt-4 space-y-1 border-t border-[#EFEAE0] pt-4 text-sm">
                ...
                {shownInsurers.length > 0 && (
                    <p className="text-[#5B6B6E]"><span className="font-semibold text-[#3A4B49]">Insurance:</span> {shownInsurers.join('  •  ')}{extraInsurers > 0 ? `  •  +${extraInsurers}` : ''}</p>
                )}
                {p.slidingScale && <p className="text-[#0E6B6A]">Sliding scale available</p>}
                {p.freeLowCost && <p className="text-[#0E6B6A]">Free / low-cost services available</p>}
                {!p.fee && !shownInsurers.length && !p.slidingScale && !p.freeLowCost && p.selfPay && <p className="text-[#5B6B6E]">Self-pay</p>}
                <AvailabilityLine availability={p.availability} />
            </div>

            <div className="mt-4 flex items-center justify-end border-t border-[#EFEAE0] pt-4">
                <Link href={`/provider/${p.id}`} className="inline-flex items-center gap-1 rounded-lg px-1 text-sm font-semibold text-[#0E7C7B] transition group-hover:gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0E7C7B]/40" aria-label={`View profile for ${p.name}`}>
                    View profile
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" aria-hidden><path d="M5 12h14M13 6l6 6-6 6" /></svg>
                </Link>
            </div>
        </article>
    );
...
function CardSkeleton() {
    return (
        <div className="rounded-2xl border border-[#E7E0D2] bg-white p-5">
            ...
            <div className="mt-4 flex gap-2"><div className="h-6 w-24 animate-pulse rounded-full bg-[#F2EDE2]" /><div className="h-6 w-16 animate-pulse rounded-full bg-[#F2EDE2]" /></div>
        </div>
    );
}
...
export default function Directory({
    ...
    // secondary geography for the selected country (location cascade, guide §2.1)
    ...
    return (
        <div className="min-h-screen bg-[#F7F3EC] text-[#1F2A2E]">
            ...
            <SeeAllAreasModal open={showAreasModal} onClose={() => setShowAreasModal(false)} selected={f.areas}
                onApply={(next) => { setShowAreasModal(false); patch({ areas: next }); }} />
                ...
                <section aria-labelledby="find-support-heading" className="rounded-2xl border border-[#E7E0D2] bg-white p-5 shadow-[0_1px_2px_rgba(20,20,20,0.04)] sm:p-6">
                    ...
                    <div className="mt-5 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="mt-3 grid gap-3 sm:grid-cols-2">
                                <LabeledSelect id="q-location" label="Country or territory" value={f.location}
                                    onChange={(v) => patch({ location: v, region: '' })} options={countries.map((c) => c.name)} placeholder="Anywhere" />
                                {f.location && regions.length > 0 && (
                                    <LabeledSelect id="q-region" label={regionLabel} value={f.region}
                                        onChange={(v) => patch({ region: v })} options={regions.map((r) => r.name)} placeholder={`All ${regionLabel.toLowerCase()}s`} />
                                )}
                            </div>
                            <label className="mt-3 inline-flex cursor-pointer items-center gap-2.5 text-sm text-[#3A4B49]">
                                <input type="checkbox" checked={f.include_virtual} onChange={(e) => patch({ include_virtual: e.target.checked })}
                                    className="h-4 w-4 rounded border-[#B9C2C0] text-[#0E7C7B] focus:ring-[#0E7C7B]" />
                                Also include virtual providers
                            </label>
                        </div>

                        {/* Common areas (6 chips) + See all */}
                        <div>
                            ...
                            <div role="group" aria-label="Common areas of support" className="mt-3 flex flex-wrap gap-2">
                                {COMMON_SUPPORT_AREAS.map((need) => {
                                    const active = f.areas.includes(need);
                                    return (
                                        <button key={need} type="button" aria-pressed={active} onClick={() => toggleArea(need)}
                                            ...
                                            {active && <span aria-hidden className="mr-1">✓</span>}{need}
                                        </button>
                                    );
                                })}
                            </div>
                            <button type="button" onClick={() => setShowAreasModal(true)} className="mt-3 text-sm font-medium text-[#0E7C7B] underline underline-offset-2 hover:text-[#0B6463]">
                                See all areas of support{f.areas.length > COMMON_SUPPORT_AREAS.filter((c) => f.areas.includes(c)).length ? ` (${f.areas.length} selected)` : ''}
                            </button>
                        </div>
                    ...
                    {showRefine && (
                        <div id="refine-panel" className="mt-6 rounded-2xl border border-[#EFE7D6] bg-[#FBF8F2] p-5">
                            ...
                            <label className="mb-4 flex cursor-pointer items-center gap-2.5 text-sm font-medium text-[#3A4B49]">
                                <input type="checkbox" checked={f.accepting} onChange={(e) => patch({ accepting: e.target.checked })}
                                    className="h-4 w-4 rounded border-[#B9C2C0] text-[#0E7C7B] focus:ring-[#0E7C7B]" />
                            ...
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                        ...
                                        {PAYMENT_OPTIONS.map((opt) => {
                                            ...
                                            return (
                                                <button key={opt.key} type="button" role="radio" aria-checked={active}
                                                    ...
                                                    className={`rounded-full border px-3.5 py-1.5 text-sm font-medium transition ${active ? 'border-[#C2543B] bg-[#F6E6DF] text-[#C2543B]' : 'border-[#DED7C9] bg-white text-[#3A4B49] hover:border-[#C2543B]/40'}`}>
                                                    {opt.label}
                                                </button>
                                            );
                                        })}
                                        <button type="button" role="radio" aria-checked={f.payment === ''} onClick={() => patch({ payment: '', insurer: '' })}
                                            className={`rounded-full border px-3.5 py-1.5 text-sm font-medium transition ${f.payment === '' ? 'border-[#C2543B] bg-[#F6E6DF] text-[#C2543B]' : 'border-[#DED7C9] bg-white text-[#3A4B49] hover:border-[#C2543B]/40'}`}>
                                            Show all options
                                        </button>
                                    </div>
                                    {f.payment === 'insurance' && (filterOptions.insurers?.length || 0) > 0 && (
                                        <div className="mt-3 max-w-xs">
                                            <LabeledSelect id="r-insurer" label="Which insurer? (optional)" value={f.insurer} onChange={(v) => patch({ insurer: v })} options={filterOptions.insurers || []} placeholder="Any insurer" />
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="sm:col-span-2 lg:col-span-3">
                                    ...
                                    <input id="r-kw" value={f.keyword} onChange={(e) => onKeyword(e.target.value)} placeholder="Name, specialty…"
                    ...
                    <p className="mb-4 text-sm text-[#5B6B6E]" aria-live="polite">
                        {loading ? 'Searching…' : (
                            <><span className="font-semibold text-[#16302F]">{total}</span>{' '}
                                {'provider' + (total === 1 ? '' : 's')}{hasAnySelection ? ' match your search' : ''}</>
                        )}
                    </p>

                    {loading ? (
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">{Array.from({ length: 4 }).map((_, i) => <CardSkeleton key={i} />)}</div>
                    ) : items.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">{items.map((p) => <ProviderCard key={p.id} p={p} selectedAreas={f.areas} />)}</div>
                    )}
