                    ...
                    {searching ? (
                        ...
                        <>
                            <div className="hidden sm:flex">
                                ...
                                <div className="grid flex-1 content-start gap-2 p-4 sm:grid-cols-2">{activeItems.map((a) => <AreaBtn key={a} area={a} />)}</div>
                            </div>
...
function AvailabilityBadge({ availability }) {
    ...
    return (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#E8B84B]/15 px-3 py-1 text-xs font-semibold text-[#9A6B12] ring-1 ring-inset ring-[#E8B84B]/40">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden><path d="M12 17h.01M12 13a2 2 0 0 0 .5-3.94A2 2 0 1 0 10 7m2 14a9 9 0 1 1 0-18 9 9 0 0 1 0 18Z" strokeLinecap="round" strokeLinejoin="round" /></svg>
...
function ProviderCard({ p, selectedAreas = [] }) {
    ...
    const Row = ({ children }) => <p className="flex items-center gap-2 text-sm text-[#3A4B49]">{children}</p>;
    ...
    return (
        <article className="flex flex-col gap-5 rounded-2xl border border-[#E7E0D2] bg-white p-5 shadow-[0_1px_2px_rgba(20,20,20,0.04)] transition hover:border-[#DCD0BA] hover:shadow-[0_10px_30px_-12px_rgba(14,124,123,0.2)] sm:flex-row">
            ...
            <div className="flex w-full flex-shrink-0 flex-col gap-2.5 border-t border-[#EFEAE0] pt-4 sm:w-52 sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0">
                ...
                {p.fee && <Row><span className="font-semibold text-[#16302F]">{p.fee}</span></Row>}
                {shownInsurers.length > 0 && <Row><span className="truncate">{shownInsurers.join(', ')}{extraInsurers > 0 ? ` + ${extraInsurers} more` : ''}</span></Row>}
                {p.slidingScale && <Row><span className="text-[#0E6B6A]">Sliding scale available</span></Row>}
                {p.freeLowCost && !p.slidingScale && <Row><span className="text-[#0E6B6A]">Free / low-cost</span></Row>}
                {!shownInsurers.length && !p.slidingScale && !p.freeLowCost && p.selfPay && <Row><span>Self-pay</span></Row>}
                {p.languages?.length > 0 && <Row><span className="truncate text-[#5B6B6E]">{p.languages.join(', ')}</span></Row>}
    ...
    return (
        <div className="space-y-4">
            <Group title="Availability">
                <Check checked={f.accepting} onChange={(v) => patch({ accepting: v })}>Accepting new clients</Check>
            </Group>

            <Group title="Location & format">
                <Check checked={f.include_virtual} onChange={(v) => patch({ include_virtual: v })}>Include virtual providers</Check>
