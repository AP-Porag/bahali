...
interface ProviderData {
    ...
    licence_verified?: boolean | number | null;
}
interface PageProps {
...
const SAMPLE: ProviderData = {
    ...
    availability_confirmed_at: 'June 24, 2026',
...
const STEPS = [
    ...
    { key: 'payment', title: 'Insurance & Payment', subtitle: 'Accepted methods & fee' },
...
/* Availability badge — mirrors the public 3-state (guide §5). */
function AvailabilityBadge({ value }: { value?: boolean | null }) {
    ...
    if (value === false) {
        return (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[#8A9795]/15 px-3 py-1 text-sm font-semibold text-[#5B6B6E]">
                Not currently accepting new clients
            </span>
        );
    }
    return (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#E8B84B]/18 px-3 py-1 text-sm font-semibold text-[#9A6B12]">
            Not set — will show as “availability unknown”
        </span>
    );
}
...
function ReviewBody({ step, p, onViewDoc }: { step: number; p: ProviderData; onViewDoc?: (url: string) => void }) {
    switch (step) {
        ...
        case 9:
            return (
                <DL>
                    ...
                    <Row label="Session fee range">{p.fee_range}</Row>
                </DL>
            );
        case 10:
            return (
        ...
        case 13:
            return (
                <DL>
                    <Row label="Accepting new clients?"><AvailabilityBadge value={p.accepting_new_clients} /></Row>
                    <Row label="Availability last confirmed">{p.availability_confirmed_at || undefined}</Row>
                </DL>
            );
        case 14:
            return (
...
export default function ProviderVerificationShow({ provider, indexRoute = 'admin.providers.index' }: PageProps) {
    ...
    const [licenceVerified, setLicenceVerified] = useState<boolean>(
        Boolean(p?.licence_verified)
    );

    // If the user navigates to a different provider without a full reload,
    // keep the checkbox in sync with that provider's saved value.
    useEffect(() => {
        setLicenceVerified(Boolean(p?.licence_verified));
    }, [p?.id, p?.licence_verified]);
    ...
    const submit = (status: string) => {
        ...
        router.post(
            ...
            {
                status, note,
                licence_verified: p.provider_type === 'individual' && status === 'approved'
                    ? licenceVerified
                    : false,
            },
                ...
                <div ref={topRef} className="mx-auto max-w-5xl px-5 py-8 lg:py-12">
                    ...
                    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            {/* {p.provider_type === 'individual' && (p as any).licence_verified && (
                                <p className="mt-1.5 inline-flex items-center gap-1.5 rounded-full bg-[#0E7C7B]/12 px-3 py-1 text-xs font-semibold text-[#0E6B6A]">
                                    <svg viewBox="0 0 20 20" className="h-3.5 w-3.5" fill="currentColor"><path d="M7.6 13.4 4.2 10l-1.2 1.2 4.6 4.6 9-9L15.4 5.6z" /></svg>
                                    Licence verified
                                </p>
                            )} */}
                            ...
                            <div className="rounded-2xl border border-[#E7E0D2] bg-white p-6 shadow-sm sm:p-8">
                                ...
                                {step === DECISION_STEP ? (
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        {/* Licence verification — only for licensed individual providers (client §5) */}
                                            ...
                                            <div className="rounded-xl border border-[#DED7C9] bg-[#FBF8F2] p-4">
                                                <label className="flex cursor-pointer items-start gap-3">
                                                    <input
                                                        ...
                                                        onChange={(e) => setLicenceVerified(e.target.checked)}
                                                        className="mt-0.5 h-5 w-5 rounded border-[#B9C2C0] text-[#0E7C7B] focus:ring-[#0E7C7B]"
                                                    />
                                                    <span>
                                                        <span className="block text-sm font-semibold text-[#26403F]">
                                                            I have verified this provider's licence against the relevant licensing authority
                                                        </span>
                                                        <span className="mt-0.5 block text-xs text-[#6B7A78]">
                                                            Check this only if you have confirmed the licence number, status, and issuing authority
                                                            (e.g. via the Florida Board of Psychology, Jamaica Psychology Board). If unchecked,
                                                            the public profile will read <strong>"Profile reviewed"</strong>
                                                        </span>
                                                    </span>
                                                </label>
                                            </div>
